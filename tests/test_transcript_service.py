...
def test_traversal_in_filename_is_rejected(synthetic_transcript_pdf):
    response = parse_transcript_bytes("../../etc/passwd.pdf", "application/pdf", synthetic_transcript_pdf)
    assert response.success is False
...
def test_llm_extraction_is_used_when_enabled(synthetic_transcript_pdf):
    ...
    with patch.object(llm_extraction, "is_enabled", return_value=True), patch.object(
        ...
        response = parse_transcript_bytes("t.pdf", "application/pdf", synthetic_transcript_pdf)
...
def test_llm_failure_falls_back_to_rules_without_failing_the_upload(synthetic_transcript_pdf):
...
def test_markitdown_failure_does_not_leave_temp_file(synthetic_transcript_pdf):
...
def test_parsing_writes_nothing_to_the_database(synthetic_transcript_pdf, db_session):
    """The privacy boundary: parsing a transcript must not touch a table."""
    from sqlalchemy import func, select

    from models.student import StudentProfile

    parse_transcript_bytes("transcript.pdf", "application/pdf", synthetic_transcript_pdf)
