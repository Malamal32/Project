...
class Posting(Base):
    ...
    posting_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ...
    company_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("companies.company_id"), nullable=False)
    title_raw: Mapped[str] = mapped_column(String(500), nullable=False)
    description_raw_ref: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("raw_documents.raw_document_id"), nullable=False
    )
...
class PostingVersion(Base):
    ...
    posting_version_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    posting_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("postings.posting_id"), nullable=False)
    title_raw: Mapped[str] = mapped_column(String(500), nullable=False)
    description_raw_ref: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("raw_documents.raw_document_id"), nullable=False
    )
